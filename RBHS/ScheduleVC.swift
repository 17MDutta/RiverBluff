...
class ScheduleVC: UIViewController, UITableViewDataSource, UITableViewDelegate, UIScrollViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        for var i = 0; i < days.count/7; i++ {
            ...
            let weekView=UIView(frame: CGRectMake(screenSize.width * CGFloat(i),0, screenSize.width, 48))
            weekView.layer.borderWidth = 0
            
            //Populate WeekView with day Buttons
            for var c = 0; c < 5; c++ {
                
                let xValue = Int(screenSize.width)/5 * (c)
                let width = Int(screenSize.width)/5
                //Format
        ...
        self.scrollView.contentSize = CGSizeMake(CGFloat(Int(screenSize.width) * (days.count/7)), 48)
    ...
    func changeViewToday() {
        ...
        scrollView.setContentOffset(CGPoint(x: 10240, y: 0), animated: true)
